...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-gradient-to-br from-navy-850 via-navy-900 to-navy-950 p-6 shadow-card sm:p-8">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          ...
          <QuickLink
            ...
            label="Agent style"
            hint="Voice, color, greeting"
          />
        </div>
      </div>

      {widgetNotice ? (
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm leading-6 text-navy-100">
          {widgetNotice.error ? (
            <>
      ...
      {isWixPlatform(session.platform) && !widgetNotice ? (
        <div className="rounded-2xl border border-white/10 bg-navy-900/50 px-4 py-3 text-sm text-navy-300">
          ...
          <div className="mt-5 space-y-3">
            {data.topQuestions.length ? (
              data.topQuestions.map((item, index) => (
                <div
                  key={item.topic}
                  className="flex items-center justify-between rounded-2xl border border-white/5 bg-navy-950/40 px-4 py-3"
                >
                  <div>
...
function QuickLink({
  ...
  return (
    <Link
      href={href}
      className="group rounded-2xl border border-white/10 bg-navy-950/40 px-4 py-3 transition hover:border-amber-500/35 hover:bg-amber-500/5"
    >
...
function Row({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="flex items-center justify-between rounded-xl bg-navy-950/35 px-3 py-2.5">
