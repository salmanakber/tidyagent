...
import { requestHumanFromWidget } from "@/modules/handoff/request";
...
export async function POST(request: Request) {
  ...
  if (!entitlements.isPaidSeat || suspended || (!parsed.preview && agent.status !== "ACTIVE")) {
    return NextResponse.json({ error: "Chat is not available right now." }, { status: 403, headers: corsHeaders() });
  }

  try {
    const result = await requestHumanFromWidget({
      agent,
    ...
    return NextResponse.json(
      {
        ...
        options: Boolean(result.options),
        ...
        agent: result.agent
          ? personPayload({
              ...result.agent,
              widgetAvatarUrl: result.agent.avatarUrl,
              voiceId: result.agent.voiceId,
            })
          : undefined,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not reach the team" },
