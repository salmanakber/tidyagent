  ...
  return (
    <div className="panel p-6">
          ...
          {notes.map((note) => (
            <li key={note.id} className="rounded-2xl bg-navy-950/40 px-4 py-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {note.sensitive ? (
                  <span className="rounded-full bg-rose-500/15 px-2 py-0.5 text-[10px] uppercase tracking-wide text-rose-200">
                    Private
                  </span>
                ) : null}
              </div>
              <p className="mt-1 whitespace-pre-wrap text-sm text-navy-300">
                {note.sensitive ? "Private instruction — used, not shown to visitors." : note.content}
              </p>
            </li>
          ))}
        </ul>
      ...
      >
        <input
          className="field"
          placeholder="Title (e.g. Seasonal rates)"
          value={title}
        ...
        <button
          ...
          disabled={pending || title.trim().length < 2 || composeOwnerNote(fields).length < 2}
        >
