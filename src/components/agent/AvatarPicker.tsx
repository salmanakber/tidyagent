    ...
    <div>
      ...
      <div className="mt-3 flex items-center gap-4">
        <button
          ...
          disabled={busy}
          className="relative h-20 w-20 overflow-hidden rounded-full ring-1 ring-white/10 transition hover:ring-amber-400/40 disabled:opacity-60"
        >
          {url ? (
            <img src={url} alt="" className="h-full w-full object-cover" />
          ) : (
            <span className="flex h-full w-full items-center justify-center bg-amber-500/15 font-display text-lg text-amber-200">
              {initials(name) || "AI"}
            </span>
          )}
